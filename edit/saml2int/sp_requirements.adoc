...
** an `<mdui:UIInfo>` extension element containing the child elements `<mdui:DisplayName>`, `<mdui:Logo>`, and `<mdui:InformationURL>`
...
* the `<mdui:UIInfo>` extension element must also contain a child element `<mdui:PrivacyStatementURL>`
