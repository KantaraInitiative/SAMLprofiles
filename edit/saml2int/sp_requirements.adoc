...
[SDP-SP13]:: The response to such errors MUST direct users to appropriate support resources offered by the SP or, alternatively, to the `errorURL` attribute in an IdP's metadata if the cause of the error is inferred to be a lack
...
[SDP-SP31]:: Requests MUST be signed (via a signature created in accordance with the HTTP=Redirect binding [SAML2Bind]).
...
[SDP-SP36]:: Responses MUST be signed (via a signature created in accordance with the HTTP=Redirect binding [SAML2Bind]).
