...
[SDP-SP28]:: SPs SHOULD consider support for the SP Request Initiation Profile and Protocol defined in <<IdPDisco>> as it provides a general, composable building block. SPs MAY support other mechanisms and caching solutions (e.g., cookies) as desired, to reduce the frequency of discovery.
