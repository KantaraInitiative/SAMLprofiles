...
[SDP-G02]:: Unless otherwise specified, deployments MUST limit the size of each individual XML element's and XML attribute's text value they produce to 256 characters. This applies in particular to the values within `<saml:NameID>` and `<saml:AttributeValue>` elements.
