...
[SDP-MD09]:: Metadata MUST include an `<mdui:UIInfo>` element as defined in <<MetaUI>> containing at least the child elements `<mdui:DisplayName>` and `<mdui:Logo>`. SP's MUST include the child element `<PrivacyStatementURL>`
