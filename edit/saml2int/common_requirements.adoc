...
[SDP-G02]:: Unless otherwise specified, deployments MUST limit the size of all XML element and attribute values they produce to 256 characters per element or attribute value. This applies in particular to the values within `<saml:NameID>` and `<saml:AttributeValue>` elements.
