...
[IIP-SP14]:: Service Providers SHOULD (or MAY, open for discussion) support the SAML V2.0 SingleLogout profile <<SAML2Prof>>, as updated by <<SAML2Errata>>. It is OPTIONAL to actually support successful processing of logout requests, but in supporting the profile, Service Providers MUST at least support the request and response exchange.
