...
- The Interop Profile introduces a new layer in its outline for requirements that are
...
=== Requirement Mapping
The June 2010 version 2.0 of the eGov profile was reformatted in a more formal way
with requirement identifiers for better reference, and is published as version 2.0bis.
The following mapping table is using these identifiers.
...
to give a guide to the changes in general. Only MUST and SHOULD requirements
